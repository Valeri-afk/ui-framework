...
namespace ui
{
    ...
    Color ToggleButton::presentationBackgroundColor() const noexcept
    {
        ...
        if (selected_)
        {
            color.r = static_cast<uint8_t>(color.r + (255 - color.r) * 0.16f);
            color.g = static_cast<uint8_t>(color.g + (255 - color.g) * 0.16f);
            color.b = static_cast<uint8_t>(color.b + (255 - color.b) * 0.16f);
        }
        return color;
    }
    ...
    Color ToggleButton::presentationBorderColor() const noexcept
    {
        ...
        if (selected_)
        {
            color.r = static_cast<uint8_t>(color.r + (255 - color.r) * 0.20f);
            color.g = static_cast<uint8_t>(color.g + (255 - color.g) * 0.20f);
            color.b = static_cast<uint8_t>(color.b + (255 - color.b) * 0.20f);
        }
        return color;
    }

    Color ToggleButton::presentationTextColor() const noexcept
    {
        return Button::presentationTextColor();
    }
}
