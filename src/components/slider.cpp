...
namespace ui
{
    ...
    void Slider::setMinimum(float minimum) noexcept
    {
        minimum_ = minimum;
    ...
    void Slider::setMaximum(float maximum) noexcept
    {
    ...
    void Slider::setValue(float value)
    {
        ...
        value_ = value;
        if (value_ != old && onValueChanged_)
            onValueChanged_(*this, value_);
    }
    ...
    void Slider::setOnValueChanged(ValueCallback callback)
    {
    ...
    void Slider::updateFromPointer(float x)
    {
        ...
        const float padding = std::min(8.0f, size.width * 0.1f);
        const float left = position.x + padding;
        const float right = position.x + size.width - padding;
        const float usable = std::max(1.0f, right - left);
        const float t = std::clamp((x - left) / usable, 0.0f, 1.0f);
        setValue(minimum_ + (maximum_ - minimum_) * t);
    }
    ...
    void Slider::draw(SDL_Renderer *renderer)
    {
        ...
        const float left = position.x + padding;
        const float right = position.x + size.width - padding;
        const float centerY = position.y + size.height * 0.5f;
        const float trackHeight = std::max(2.0f, size.height * 0.18f);

        primitives::roundedBoxRGBA(renderer,
                                   left, centerY - trackHeight * 0.5f,
                                   right, centerY + trackHeight * 0.5f,
                                   trackHeight * 0.5f,
                                   110, 110, 110, 255);

        const float range = maximum_ - minimum_;
        const float t = range > 0.0f ? (value_ - minimum_) / range : 0.0f;
        const float thumbX = left + (right - left) * std::clamp(t, 0.0f, 1.0f);
        const float thumbRadius = std::max(5.0f, size.height * 0.35f);

        primitives::filledCircleRGBA(renderer, thumbX, centerY, thumbRadius,
                                     230, 230, 230, 255);
    }

    void Slider::handleMouseDown(MouseDownEvent &event)
    {
        if (event.button != MouseButton::Left || !isEnabled())
            return;
        dragging_ = true;
        updateFromPointer(event.position.x);
    }

    void Slider::handleMouseMove(MouseMoveEvent &event)
    {
        if (dragging_ && isEnabled())
            updateFromPointer(event.position.x);
    }

    void Slider::handleMouseUp(MouseUpEvent &event)
    {
