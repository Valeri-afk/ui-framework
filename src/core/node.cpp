...
namespace
{
    ...
    ui::LayoutSizeValue sanitizeSizeValue(
        ui::LayoutSizeValue size) noexcept
    {
        if (size.width.type == ui::LayoutValueType::Value)
        {
            size.width.value =
                finiteOrZero(size.width.value);
        }

        if (size.height.type == ui::LayoutValueType::Value)
        {
            size.height.value =
                finiteOrZero(size.height.value);
        }

        return size;
    }

    ui::LayoutPosition sanitizePosition(ui::LayoutPosition position) noexcept
    {
        return {
    ...
    void keepMaxAtLeastMin(
        ui::LayoutSize &minSize,
        ui::LayoutSize &maxSize) noexcept
    {
        maxSize.width =
            std::max(finiteOrInfinity(maxSize.width), minSize.width);

        maxSize.height =
            std::max(finiteOrInfinity(maxSize.height), minSize.height);
    }

    void keepMinAtMostMax(
        ui::LayoutSize &minSize,
        ui::LayoutSize &maxSize) noexcept
    {
        minSize.width =
            std::min(finiteOrInfinity(minSize.width), maxSize.width);

        minSize.height =
            std::min(finiteOrInfinity(minSize.height), maxSize.height);
    }

    void setMinWidthValue(
        ui::LayoutSize &minSize,
        ui::LayoutSize &maxSize,
...
namespace ui
{
    ...
    void Node::deferLayoutMutation(std::function<void(Node &)> fn)
    {
        ...
        tree->enqueueNodeMutation(
            *this,
            [tree, nodeId, fn = std::move(fn)]() mutable
            {
    ...
    Node::Id Node::id() const noexcept
    {
        return id_;
    }
    ...
    void Node::setEnabled(bool enabled) noexcept
    {
        enabled_ = enabled;
    }
    ...
    void Node::setFocusable(bool focusable) noexcept
    {
        focusable_ = focusable;
    }

    bool Node::isFocusable() const noexcept
    {
        return focusable_;
    }

    void Node::setCapturable(bool capturable) noexcept
    {
        capturable_ = capturable;
    }

    bool Node::isCapturable() const noexcept
    {
        return capturable_;
    }
    ...
    void Node::setPosition(const LayoutPosition &position)
    {
        ...
        deferLayoutMutation(
            [safePosition](Node &node) mutable
            {
                node.position_ = safePosition;
            });
    }

    LayoutPosition Node::getPosition() const noexcept
    {
        return position_;
    }
    ...
    {
        deferLayoutMutation(
            [positionMode](Node &node) mutable
            {
    ...
    void Node::setSize(
        const LayoutSizeValue &size)
    {
        const LayoutSizeValue safe =
            sanitizeSizeValue(size);
    ...
    LayoutSizeValue Node::getSize() const noexcept
    {
        return size_;
    }

    LayoutSize Node::getDesiredSize() const noexcept
    {
        return desiredSize_;
    }
    ...
    void Node::setMinSize(const LayoutSize &size)
    {
        ...
        deferLayoutMutation(
            [safeSize](Node &node)
            {
                node.minSize_ = safeSize;

                keepMaxAtLeastMin(node.minSize_, node.maxSize_);
            });
    }
    ...
    void Node::setMaxSize(const LayoutSize &size)
    {
        ...
        deferLayoutMutation(
            [safeSize](Node &node)
            {
                node.maxSize_ = safeSize;

                keepMinAtMostMax(node.minSize_, node.maxSize_);
            });
    }
    ...
    void Node::setMinWidth(float width)
    {
        ...
        deferLayoutMutation(
            [safeWidth](Node &node)
            {
                setMinWidthValue(node.minSize_, node.maxSize_, safeWidth);
            });
    }
    ...
    void Node::setMinHeight(float height)
    {
        ...
        deferLayoutMutation(
            [safeHeight](Node &node)
            {
                setMinHeightValue(node.minSize_, node.maxSize_, safeHeight);
            });
    }
    ...
    void Node::setMaxWidth(float width)
    {
        ...
        deferLayoutMutation(
            [safeWidth](Node &node)
            {
                setMaxWidthValue(node.minSize_, node.maxSize_, safeWidth);
            });
    }
    ...
    void Node::setMaxHeight(float height)
    {
        ...
        deferLayoutMutation(
            [safeHeight](Node &node)
            {
                setMaxHeightValue(node.minSize_, node.maxSize_, safeHeight);
            });
    }

    LayoutSize Node::getMinSize() const noexcept
    {
        return minSize_;
    }

    LayoutSize Node::getMaxSize() const noexcept
    {
        return maxSize_;
    }

    float Node::getMinWidth() const noexcept
    {
        return finiteOrZero(minSize_.width);
    }

    float Node::getMinHeight() const noexcept
    {
        return finiteOrZero(minSize_.height);
    }

    float Node::getMaxWidth() const noexcept
    {
        return finiteOrInfinity(maxSize_.width);
    }

    float Node::getMaxHeight() const noexcept
    {
        return finiteOrInfinity(maxSize_.height);
    }

    LayoutSize Node::clampSize(
        LayoutSize size,
        LayoutSize minSize,
    ...
    Padding Node::getPadding() const noexcept
    {
        return padding_;
    }
    ...
    void Node::setLeftPadding(float value)
    {
        deferLayoutMutation(
            [value](Node &node)
            {
                node.padding_.left = finiteOrZero(value);
            });
    }
    ...
    void Node::setRightPadding(float value)
    {
        deferLayoutMutation(
            [value](Node &node)
            {
                node.padding_.right = finiteOrZero(value);
            });
    }
    ...
    void Node::setTopPadding(float value)
    {
        deferLayoutMutation(
            [value](Node &node)
            {
                node.padding_.top = finiteOrZero(value);
            });
    }
    ...
    void Node::setBottomPadding(float value)
    {
        deferLayoutMutation(
            [value](Node &node)
            {
                node.padding_.bottom = finiteOrZero(value);
            });
    }
    ...
    void Node::setBorder(const Border &border)
    {
        deferLayoutMutation(
            [border](Node &node)
            {
                node.border_ = sanitizeBorder(border);
            });
    }

    Border Node::getBorder() const noexcept
    {
        return border_;
    }
    ...
    void Node::setLeftBorder(float value)
    {
        deferLayoutMutation(
            [value](Node &node)
            {
                node.border_.left = finiteOrZero(value);
            });
    }
    ...
    void Node::setRightBorder(float value)
    {
        deferLayoutMutation(
            [value](Node &node)
            {
                node.border_.right = finiteOrZero(value);
            });
    }
    ...
    void Node::setTopBorder(float value)
    {
        deferLayoutMutation(
            [value](Node &node)
            {
                node.border_.top = finiteOrZero(value);
            });
    }
    ...
    void Node::setBottomBorder(float value)
    {
        deferLayoutMutation(
            [value](Node &node)
            {
                node.border_.bottom = finiteOrZero(value);
            });
    }
    ...
    void Node::setOverflow(Overflow overflow)
    {
        deferLayoutMutation(
            [overflow](Node &node)
            {
                node.overflow_ = overflow;
            });
    }

    Overflow Node::getOverflow() const noexcept
    {
        return overflow_;
    }
